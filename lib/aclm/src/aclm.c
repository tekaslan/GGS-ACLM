...
int runEmergencyPlanning(SearchProblem *problem, char * casefolder, char * configfile)
{
    ...
    if (problem->solver == SEARCH_AIRSPACE) {
        ...
        while (((problem->exitFlag == 2 || problem->exitFlag == -3) || problem->siteIndex == -1) && (problem->siteIndex < problem->numRanked - 1)) {
            ...
            // Run the gradient-guided search for emergency landing planning
            runSearch(problem);
...
int readGammaStraight(struct Aircraft *ac, char *ac_name)
{
    ...
    FILE * file;
    char dir[300];
    sprintf(dir,"/Users/hetekaslan/Desktop/AirDeconfliction/search_aclm_c/data/gamma_straight.dat");
    file = fopen(dir,"r");
    ...
    sprintf(dir,"/Users/hetekaslan/Desktop/AirDeconfliction/search_aclm_c/data/gamma_straight_100kts.dat");
    file = fopen(dir,"r");
...
/*
    ...
    It cannot identify SEARCH_AIRSPACE.
*/
...
int selectLandingSite(SearchProblem *problem) {
    ...
    // printf("Ranked Landing Sites:\n");
    // for (int i = 0; i < problem->numRanked; i++) {
    //     landingSite *site = problem->rankedSites[i];
    //     printf("  [%d], ID = %d | U = %.3f | γ = %.2f° | Length = %.0f ft | Width = %.0f ft | Commercial = %.0f | Military = %.0f\n",
    //         i + 1, site->ident, site->U, site->gamma, site->length, site->width, site->commercial, site->military);
    // }
...
/*
    Computes cumulative ground risk
*/
...
/*
    Computes cumulative airspace risk
*/
...
void freeRankedSites(struct landingSite **rankedSites, int count) {
    ...
    // // Just clear the pointers; do not free nodes again
    // for (int i = 0; i < count; ++i) {
    //     free(rankedSites[i]); rankedSites[i] =  NULL;
    // }
