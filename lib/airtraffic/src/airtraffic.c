    ...
    while (tmp_corridor && flag)
    {
        ...
        while (tmp_segment && flag) {
            ...
            for (; i < 6; i++) {
                ...
                // Due to truncation and round-off errors, instead of zero, a small positive value is used.
                if (dot_prod[i] > 1e-6)
                {
                    break;
                }
...
double minimum_distance(struct Pos *pos, struct airCorridor *corridors)
{
    ...
    if (!flag_isinside) {
        d_min_corridors = 0;
    } else {   // If not, search for the minimum distance
...
{   
    ...
    double max_d = 0.164578833693305/2; // [NM] ~ 1000 ft
    if (d > max_d) d = max_d;
    return 1.0 - d / max_d;
}