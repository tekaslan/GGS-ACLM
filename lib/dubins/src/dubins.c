                                    ...
                                    struct GeoOpt *GeoOpt)
{
    ...
    // Initialize the angle between straight segments
    double theta = 60; // [deg]
    ...
    for (int i = 0; i < 2; i++) {
        ...
        sTurnPath[i].size = 2;                  // Sets path structure size to 2, indicating the structure holds two different Dubins paths
    }

    // Iterate over theta
    int maxIter = 1000;
    int counter = 0;
    while ((fabs(dh - dAltitude) > 3) && (theta < 180) && (counter < maxIter)) {
        ...
        sTurnPath[0].hdist = 0;
        for (int i = 0; i < 3; i++) {
            sTurnPath[0].hdist += sTurnPath[0].traj[i].hdist;
        }
        ...
        sTurnPath[1].hdist = 0;
        for (int i = 0; i < 3; i++) {
            sTurnPath[1].hdist += sTurnPath[1].traj[i].hdist;
        }
                                    ...
                                    double *riskRuntime)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (fabs(alt_change - dAltitude) > 5) continue;
    ...
    if (!feasible || bestIdx == -1) {
        for (int i = 0; i < 2; i++) {
            ...
            bestSturn[i].gp = INFINITY;
            bestSturn[i].ga = INFINITY;
            bestSturn[i].risk = INFINITY;
                                        ...
                                        double *riskRuntime)
{
    ...
    if (dist*NM_2_FT < 3*problem->ac->turnRadius){
...
{   
    ...
    double d, course;
    geo_dist(&problem->InitialSearch, &dubins->traj[3].wpt.pos, &d, &course, &problem->GeoOpt);
    
    double dist = 0;
    double initalt = dubins->traj[3].wpt.pos.alt;
    while (d*NM_2_FT < 3*problem->ac->turnRadius) {
        dist += problem->ac->turnRadius*FT_2_NM;
        course = wrapTo360(dubins->traj[3].wpt.hdg - 180);
    ...
    dubins->traj[3].wpt.pos.alt = initalt + dist*tan(gammaArray[0]*DEG_2_RAD);  // Best-glide angle
    free(gammaArray); gammaArray = NULL;
}
